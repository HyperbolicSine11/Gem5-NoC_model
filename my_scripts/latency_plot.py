...
if __name__ == "__main__":
    ...
    ax1.bar(
        ...
        color="blueviolet",
        width=width,
        label="average_packet_network_latency",
    )
    ...
    for a, b in zip(x - width / 2, avg_pkt_net_latency):
        ax1.text(a, b, "%.1f" % b, ha="center", va="bottom", fontsize=13)
    ...
    for a, b in zip(x + width / 2, avg_hops):
        ax2.text(a, b, "%.1f" % b, ha="center", va="bottom", fontsize=13)
    ax2.tick_params(axis="y", labelsize=20)
    ax2.set_ylabel("Hops", fontsize=24)

    plt.title("Average Packet Latency & Average hops", fontsize=20)
    plt.xticks(x, labels)
    fig.autofmt_xdate(rotation=30)
    plt.legend(loc="upper right", fontsize=18)
    plt.savefig("LLama2_Attention_FC.png")
    # plt.show()
